 ...
 * Formula: (butterUsdPerLb * 3) / nightUsd = $NIGHT per pound of butter in the US.
 * Example: (1.5750 * 3) / 0.05406 = 87.402 $NIGHT per LB
 */
function mergeChartData(
  ...
  for (const { date, close } of historical) {
    ...
    const butterUsdPerLb = prev ? butterByDate.get(prev)! : 0;
    if (butterUsdPerLb <= 0 || close <= 0) continue;
    const ratio = (butterUsdPerLb * 3) / close;
    out.push({
      date,
      butterUsdPerLb,
      nightUsd: close,
